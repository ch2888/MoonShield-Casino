...
contract Slot is SlotOwnable, Initializable {
    ...
    */
    ...
    function calculatePrize(uint256 linelength, uint256 coinamount) public returns(uint256) {
        
        uint256 r = randomProbability();
        uint256 reward = 0;
        uint256 j = 0;
        
        if (r <= 9000){
            j = randomGen(2, linelength);
        } else if((r > 9000) && (r <= 9900)){
            j = randomGen(5, linelength);
        } else if((r > 9900) && (r <= 9990)){
            j = randomGen(10, linelength);
        } else if (r > 9990){
            j = randomGen(15, linelength);
        }
        reward = weightvalue[j];
        
        reward = reward * coinamount;
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            gen.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function emergencyWithdraw() public {
        ...
        user.amount = 0;
        gen.safeTransfer(address(msg.sender), amount);
    ...
    function randomGen(uint256 seed, uint256 linelength) public returns (uint randomNumber) { 
        ...
        */
        value1 = seed;
        value2 = keccak256(abi.encodePacked(block.timestamp*block.difficulty*block.number*timeRestriction+1));
        
        value4 = uint256(keccak256(abi.encodePacked(block.timestamp*block.difficulty*block.number*timeRestriction+1)));
        
        string memory strr = integerToString(value4);
        value5 = strr;
        
        bytes memory bs = bytes(strr);
        value6 = bs.length;
        
        value7 = value6/linelength;
        
        string memory strvar = new string(linelength);
        
        for(uint256 i = 0; i < linelength; i++){
            bytes memory newString = new bytes(value7);
            for(uint256 j = 0; j < value7; j++){
                uint256 k = i * value7 + j;
                newString[j] = bs[k];
            }
        ...
        value14 = uint256(keccak256(abi.encodePacked(block.timestamp*block.difficulty*block.number*timeRestriction+1))) % seed;
        return (uint256(keccak256(abi.encodePacked(block.timestamp*block.difficulty*block.number*timeRestriction+1))) % seed);
    }
    ...
    function randomProbability() public returns (uint randomNumber) {
        
        value15 = uint256(keccak256(abi.encodePacked(block.timestamp*block.difficulty*block.number*timeRestriction+1))) % 10001;
        return (uint256(keccak256(abi.encodePacked(block.timestamp*block.difficulty*block.number*timeRestriction+1))) % 10001);
    }
